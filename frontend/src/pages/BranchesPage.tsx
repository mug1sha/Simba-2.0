...
const BranchesPage = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        if (nearest) {
          ...
          toast.success(`Nearest branch found: ${nearest.name}.`, { duration: 1800 });
        }
      },
      () => {
        setLocationStatus("denied");
        toast.error("Location access was denied. Distance ranking is unavailable.");
      },
      {
  ...
  const chooseBranch = (branchName: string) => {
    if (items.length > 0 && selectedBranch && selectedBranch !== branchName) {
      toast.error(`Your cart is already locked to ${selectedBranch}. Clear it before switching branches.`);
      return;
    }
    setSelectedBranch(branchName);
    toast.success(`${branchName} is now your shopping branch.`, { duration: 1500 });
  };
        ...
        <section className="grid gap-6 lg:grid-cols-[1.02fr_0.98fr]">
          ...
          >
            <div className="space-y-5">
              <div>
                <p className="text-[10px] font-black uppercase tracking-[0.35em] text-primary">Branches</p>
                <h1 className="mt-3 text-4xl font-black tracking-tight text-foreground">
                  Tap a branch. See the route inside the page.
                </h1>
                <p className="mt-4 max-w-2xl text-sm leading-relaxed text-muted-foreground">
                  Branch cards now push directions into an embedded map below, with route distance and ETA kept on the same screen.
                </p>
              </div>
              ...
              <div className="rounded-[1.75rem] border border-primary/20 bg-primary/10 p-4">
                <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                  <div className="space-y-1">
                    <p className="text-[10px] font-black uppercase tracking-[0.25em] text-primary">Your location</p>
                    {userLocation ? (
                      <>
                        ...
                        {nearestBranch && nearestBranch.distanceMeters !== null && (
                          <p className="text-sm text-muted-foreground">
                            Closest branch: <span className="font-bold text-foreground">{nearestBranch.name}</span> at{" "}
                            <span className="font-bold text-primary">{formatDistance(nearestBranch.distanceMeters)}</span>
                          </p>
                        )}
                      </>
                    ) : (
                      <p className="text-sm text-muted-foreground">
                        Allow location once and each branch click will redraw directions in the embedded map.
                      </p>
                    )}
                  </div>
                  <button
                ...
                {locationStatus === "denied" && (
                  <p className="mt-3 text-xs text-amber-200">
                    Location access is blocked, so route ranking is unavailable.
                  </p>
                )}
                {locationStatus === "unsupported" && (
                  <p className="mt-3 text-xs text-amber-200">This browser does not support geolocation.</p>
                )}
              </div>
              ...
              <div className="grid gap-3 sm:grid-cols-4">
                {[
                  {
                    icon: Navigation,
                    label: activeRoute ? formatKilometers(activeRoute.routeMeters) : "Route ready",
                    caption: activeRoute ? "Suggested road distance" : "Enable location",
                  },
                  {
                    icon: Clock,
                    label: activeRoute ? `${activeRoute.driveMinutes} min` : "Fast ETA",
                    caption: activeRoute ? "Estimated drive time" : "Shown after location",
                  },
                  {
                    icon: Boxes,
                    label: `${topInventory.length || 20}+ picks`,
                    caption: "Visual branch inventory",
                  },
                  {
                    icon: CheckCircle2,
                    label: `${(ratingByBranch[activeBranch.name]?.average_rating || 4.6).toFixed(1)}★`,
                    caption: `${ratingByBranch[activeBranch.name]?.review_count || 0} branch ratings`,
                  },
          ...
          >
            <div className="relative h-[420px] overflow-hidden bg-[radial-gradient(circle_at_top,#1e293b_0%,#0f172a_38%,#020617_100%)]">
              <div className="absolute left-4 top-4 z-10 rounded-2xl border border-white/10 bg-[#08081a]/90 p-4 text-white shadow-xl backdrop-blur-md">
                <p className="text-[10px] font-black uppercase tracking-[0.25em] text-primary">Interactive map</p>
                <p className="mt-2 max-w-[260px] text-xs leading-relaxed text-gray-300">
                  Click a marker to preview that branch route. The line follows your selected branch.
                </p>
              </div>
                    ...
                    <div className="absolute inset-x-0 bottom-0 p-4 text-white">
                      ...
                      <p className="mt-1 text-[11px] font-bold text-orange-100/90">
                        ...
                        {ratingByBranch[branch.name]?.review_count
                          ? ` • ${ratingByBranch[branch.name].review_count} ratings`
                          : " • New branch"}
                      </p>
                    </div>
                  </div>
                ...
                <div className="p-5">
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <div className="flex flex-col items-end gap-2">
                      ...
                      <span className="rounded-full bg-amber-500/10 px-2 py-1 text-[10px] font-black uppercase tracking-widest text-amber-600 dark:text-amber-300">
                        ★ {(ratingByBranch[branch.name]?.average_rating || 4.6).toFixed(1)}
                        {ratingByBranch[branch.name]?.review_count ? ` (${ratingByBranch[branch.name].review_count})` : " (new)"}
                      </span>
                      {isSelected && (
                  ...
                  <div className="mt-4 grid gap-3 sm:grid-cols-2">
                    <div className="rounded-2xl border border-border/50 bg-background/70 p-3">
                      <p className="text-[10px] font-black uppercase tracking-[0.25em] text-primary">Route</p>
                      <p className="mt-2 text-sm font-black text-foreground">
                        {routeEstimate ? formatKilometers(routeEstimate.routeMeters) : "Enable location"}
                      </p>
                      <p className="mt-1 text-xs text-muted-foreground">
                        {routeEstimate ? `${formatMetersOnly(routeEstimate.routeMeters)} suggested road route` : "Distance appears after location permission."}
                      </p>
                    </div>
                    <div className="rounded-2xl border border-border/50 bg-background/70 p-3">
                      <p className="text-[10px] font-black uppercase tracking-[0.25em] text-primary">ETA</p>
                      <p className="mt-2 text-sm font-black text-foreground">
                        {routeEstimate ? `${routeEstimate.driveMinutes} min drive` : "Waiting for route"}
                      </p>
                      <p className="mt-1 text-xs text-muted-foreground">
                        {routeEstimate ? `${routeEstimate.walkMinutes} min on foot` : "Fast trip estimate appears here."}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="mt-5 flex flex-wrap gap-2">
                    ...
                    >
                      {isActive ? "Viewing Route" : "View Route"}
                    </button>
                    <button
                      type="button"
                  ...
                  >
                    Location source: {branch.sourceLabel}
                  </a>
                </div>
              </motion.article>
            );
          ...
          <div className="space-y-6">
            <div className="overflow-hidden rounded-[2rem] border border-border/50 bg-card shadow-2xl shadow-black/5">
              <div className="relative h-[520px] overflow-hidden bg-[radial-gradient(circle_at_top,#172554_0%,#111827_42%,#020617_100%)]">
                <iframe
                  ...
                  title={`${activeBranch.name} directions map`}
                  loading="lazy"
                ...
                <div className="absolute left-4 top-4 z-10 rounded-2xl border border-white/10 bg-[#08081a]/90 p-4 text-white shadow-xl backdrop-blur-md">
                  ...
                  <p className="mt-2 max-w-[280px] text-xs leading-relaxed text-gray-300">
                    Embedded directions stay inside the page. Click another branch card or marker to redraw this map.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="rounded-[2rem] border border-border/50 bg-card p-5 shadow-2xl shadow-black/5">
                <p className="text-[10px] font-black uppercase tracking-[0.25em] text-primary">Route details</p>
                {activeRoute ? (
                  <div className="mt-4 space-y-3">
                    <div className="rounded-2xl border border-border/50 bg-background/70 p-4">
                      <p className="text-xs font-black uppercase tracking-[0.2em] text-muted-foreground">Suggested route</p>
                    ...
                    <div className="grid gap-3 sm:grid-cols-2">
                      <div className="rounded-2xl border border-border/50 bg-background/70 p-4">
                        <p className="text-xs font-black uppercase tracking-[0.2em] text-muted-foreground">Direct line</p>
                        <p className="mt-2 text-lg font-black text-foreground">{formatKilometers(activeRoute.directMeters)}</p>
                      </div>
                      <div className="rounded-2xl border border-border/50 bg-background/70 p-4">
                        <p className="text-xs font-black uppercase tracking-[0.2em] text-muted-foreground">ETA</p>
                        <p className="mt-2 text-lg font-black text-foreground">{activeRoute.driveMinutes} min</p>
                        <p className="mt-1 text-xs text-muted-foreground">{activeRoute.walkMinutes} min walk</p>
                      </div>
                    </div>
                    <button
                      type="button"
                  ...
                  <div className="mt-4 rounded-2xl border border-dashed border-border/50 bg-background/60 p-6 text-sm text-muted-foreground">
                    Turn on location and click any branch to see the route, kilometers, meters, and estimated minutes.
                  </div>
                )}
              </div>

              <div className="rounded-[2rem] border border-border/50 bg-card p-5 shadow-2xl shadow-black/5">
                <p className="text-[10px] font-black uppercase tracking-[0.25em] text-primary">Visual guide</p>
                <div className="mt-4 space-y-3">
                  <div className="flex items-start gap-3 rounded-2xl border border-border/50 bg-background/70 p-4">
                    ...
                    <div>
                      <p className="text-sm font-black text-foreground">Selected branch</p>
                      <p className="text-xs leading-relaxed text-muted-foreground">Orange stays locked on the branch you clicked.</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-3 rounded-2xl border border-border/50 bg-background/70 p-4">
                    <span className="mt-1 h-4 w-4 rounded-full border-4 border-white bg-[#3b82f6]" />
                    <div>
                      <p className="text-sm font-black text-foreground">Your location</p>
                      <p className="text-xs leading-relaxed text-muted-foreground">Blue comes from your device and powers route estimates.</p>
                    </div>
                  </div>
                  <div className="rounded-2xl border border-primary/20 bg-primary/10 p-4 text-xs leading-relaxed text-muted-foreground">
                    <p>
                      <span className="font-black text-foreground">Animated path:</span> redrawn every time you switch branch.
                    </p>
                    <p className="mt-2">
                      <span className="font-black text-foreground">ETA:</span> calculated from the estimated road route for a quick planning view.
                    </p>
                  </div>
                </div>
              </div>
          ...
          <div className="rounded-[2rem] border border-border/50 bg-card p-6 shadow-2xl shadow-black/5">
            <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
              <div>
                ...
                <h2 className="mt-2 text-3xl font-black tracking-tight text-foreground">{activeBranch.name} Store</h2>
                <p className="mt-2 text-sm text-muted-foreground">
                  Fast preview of branch visuals, stock, and route before you commit the cart.
                </p>
                <p className="mt-2 text-xs font-black uppercase tracking-[0.22em] text-amber-600 dark:text-amber-300">
                  ...
                  {ratingByBranch[activeBranch.name]?.review_count
                    ? ` from ${ratingByBranch[activeBranch.name].review_count} branch reviews`
                    : " from early branch shoppers"}
                </p>
              </div>
            ...
            {branchLocked && (
              <div className="mt-5 rounded-2xl border border-amber-500/20 bg-amber-500/10 p-4 text-sm text-amber-200">
                Your cart is currently locked to {selectedBranch}. Clear the cart before switching to {activeBranch.name}.
              </div>
            )}
            ...
            <div className="mt-6 grid gap-4 lg:grid-cols-[1.05fr_0.95fr]">
              <div className="relative min-h-[260px] overflow-hidden rounded-[1.8rem] border border-border/50 bg-black">
                ...
                <div className="relative flex h-full flex-col justify-between p-5 text-white">
                  <div>
                    <p className="text-[10px] font-black uppercase tracking-[0.28em] text-orange-200">Branch moodboard</p>
                    <p className="mt-3 text-3xl font-black tracking-tight">{activeBranch.neighborhood}</p>
                    <p className="mt-2 max-w-sm text-sm text-white/75">{activeBranch.description}</p>
                  </div>
              ...
              <div className="grid gap-3">
                {[
                  {
                    label: "Road route",
                    value: activeRoute ? formatKilometers(activeRoute.routeMeters) : "Location needed",
                    caption: activeRoute ? formatMetersOnly(activeRoute.routeMeters) : "Turn on location",
                  },
                  {
                    label: "ETA",
                    value: activeRoute ? `${activeRoute.driveMinutes} min drive` : "Quick estimate",
                    caption: activeRoute ? `${activeRoute.walkMinutes} min walk` : "Shown after route lock",
                  },
                  {
                    label: "Top picks",
                    value: `${topInventory.length || 0} items`,
                    caption: "Currently visible from this branch",
                  },
                  {
                    label: "Branch rating",
                    value: `${(ratingByBranch[activeBranch.name]?.average_rating || 4.6).toFixed(1)}★`,
                    caption: ratingByBranch[activeBranch.name]?.review_count
                      ? `${ratingByBranch[activeBranch.name].review_count} customer reviews`
                      : "No reviews yet",
                  },
                ].map((card) => (
            ...
            <div className="mt-6 space-y-3">
              {isLoading ? (
                ...
                <div className="rounded-2xl border border-dashed border-border/50 bg-background/60 p-10 text-center text-sm font-bold text-muted-foreground">
                  No in-stock products found for this branch.
                </div>
              ) : (
                  ...
                  >
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        toast.success(`${item.product.name} added from ${activeBranch.neighborhood}.`, { duration: 1500 });
                      }}
